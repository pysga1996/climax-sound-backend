...
FROM (
         ...
         WHERE (:name IS NULL OR
                LOWER(name) LIKE CONCAT('%', :name, '%') OR
                LOWER(unaccent_name) LIKE CONCAT('%', :unaccentName, '%'))
           AND (:songId IS NULL OR
                a.id IN (SELECT artist_id FROM song_artist WHERE song_id = :songId))
           AND (:birthDateFrom IS NULL OR a.birth_date > :birthDateFrom)
           AND (:birthDateTo IS NULL OR a.birth_date < :birthDateTo)
     ) AS list
